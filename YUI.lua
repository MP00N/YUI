...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'Rayfield 键绑定'},
		-- buildwarnings
		-- rayfieldprompts
...
if requestsDisabled then
	overrideSetting("系统", "使用分析", false)
end

local HttpService = getService('HTTP服务')
local RunService = getService('运行服务')
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"Rayfield Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"键绑定","Set":null,"CallOnChange":true,"回拨":null,"CurrentKeybind":"K"}}},"系统":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if not success then warn('Rayfield在创建设置时遇到问题.') end
...
if Topbar:FindFirstChild('设置') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Rayfield设置'])
		end)
	end)
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "mp“，content=”我们无法启用配置保存，因为您使用的软件不支持文件系统.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "MP“，content=”此脚本的配置文件已从以前的会话中加载", Image = 4384403532})
		elseif not success and not notified then
			warn('Rayfield Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "MP", Content = "正确加载您的配置时遇到问题.\n\n有关详细信息，请查看开发人员控制台.", Image = 4384402990})
		end
	end
